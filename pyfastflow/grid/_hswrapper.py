...
def compute_hillshade(grid, altitude_deg=45.0, azimuth_deg=315.0, z_factor=1.0, 
                     multidirectional=False, azimuths_deg=None, weights=None, style=None):
    """
    ...
    if multidirectional or azimuths_deg is not None:
        return _compute_multidirectional(grid, altitude_deg, z_factor, azimuths_deg, weights)
...
def _compute_single_direction(grid, altitude_deg, azimuth_deg, z_factor):
    ...
    with pf.pool.temp_field(ti.f32, (grid.nx * grid.ny,)) as hillshade_field:
        ...
        hs.hillshade_vectorized(grid.z.field, hillshade_field.field, zenith_rad, azimuth_rad, z_factor)
...
def _compute_multidirectional(grid, altitude_deg, z_factor, azimuths_deg, weights):
    ...
    if len(weights) != len(azimuths_deg):
        raise ValueError(f"weights length ({len(weights)}) must match azimuths_deg length ({len(azimuths_deg)})")
