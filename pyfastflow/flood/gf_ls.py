...
@ti.kernel
def init_LS_on_hw_from_variable_effective_prec(hw:ti.template(), z:ti.template(), rate:ti.template()):
	"""
	...
	for i in z:
		...
		if(top > -1):  # Check if top neighbor exists
			...
			if ((hw[i] > 0 or hw[top] > 0)):
				...
				hflow = ti.math.max(z[i]+hw[i], z[top]+hw[top]) - ti.math.max(z[top], z[i])
		...
		if (left > -1):  # Check if left neighbor exists
			...
			if ((hw[i] > 0 or hw[left] > 0)):
				...
				# Surface elevation = bed + water, effective depth = max_surface - max_bed
				hflow = ti.math.max(z[i]+hw[i], z[left]+hw[left]) - ti.math.max(z[i], z[left])
