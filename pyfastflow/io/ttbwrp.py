...
def gridobj_to_grid(go):
    ...
    # Validate input
    if not hasattr(go, 'columns') or not hasattr(go, 'rows') or not hasattr(go, 'cellsize'):
        raise ValueError("Invalid GridObj: missing required attributes (columns, rows, cellsize)")
