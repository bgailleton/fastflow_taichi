...
def propag_basin_route_all(N: int, basin_route: ti.template()):
    ...
    for id in range(N):
        ...
        changes += 1
    if changes > 0:
        print(f"    DEBUG: propag_basin_route made {changes} changes")
...
def run_lakeflow_algorithm(flow_fields: BasicFlowFields, lakeflow_fields: LakeflowFields, 
    ...
    @ti.kernel 
    def init_basin_route_from_rcv():
        for i in range(N):
            lakeflow_fields.basin_route[i] = flow_fields.rcv[i]
    init_basin_route_from_rcv()
    ...
    if carve:
        ...
        @ti.kernel
        def convert_to_int32():
            for i in range(N):
                flow_fields.rcv_i32[i] = flow_fields.rcv[i]
                flow_fields.W_i32[i] = flow_fields.W[i]
        convert_to_int32()
