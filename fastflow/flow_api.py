...
class FastFlowProcessor:
    ...
    def route_depressions(self, method: str = 'carve', num_iter: int = 0):
        ...
        if self.lakeflow_fields is None:
            ...
            S = max(self.N // 10, 10000000)  # At least 100 for safety
        ...
        try:
            ...
            run_lakeflow_algorithm(self.flow_fields, self.lakeflow_fields, 
                                 self.bignum, carve, 0)  # num_iter=0 for first iteration
            ...
            if carve:
                ...
                def swap_rcv_arrays():
                    for i in range(self.N):
                        temp = self.flow_fields.rcv[i]
                        self.flow_fields.rcv[i] = self.flow_fields.rcv_[i]
                        self.flow_fields.rcv_[i] = temp
                swap_rcv_arrays()
            ...
            print(f"  After iteration 0: Found {S_iter1} remaining local minima")
