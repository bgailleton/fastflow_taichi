...
class FastFlow:
    def __init__(self, width: int, height: int):
        ...
        self.source_buffer = ti.field(ti.i32, shape=(self.n_cells,))  # ping-pong + iteration
    ...
    def initialize_flow_routing(self, precipitation: ti.template(), discharge_a: ti.template(),
        ...
        for idx in range(n_cells):
            ...
            source_buffer[idx] = 0  # Start with buffer A
    ...
    def rake_compress_iteration(self, discharge_a: ti.template(), discharge_b: ti.template(),
        ...
        for idx in range(n_cells):
            ...
            sign_bit:ti.u64 = source_buffer[idx] & ti.u64(0x80000000)
            last_iter:ti.u64 = source_buffer[idx] & ti.u64(0x7FFFFFFF)
            
            read_from_a = 1
            if last_iter == iteration:
                read_from_a = 1 - ((sign_bit >> 31) & 1)
            else:
                read_from_a = (sign_bit >> 31) & 1
            
            # Read current state
            current_discharge = discharge_a[idx] if read_from_a else discharge_b[idx]
            current_recipient = recipients_a[idx] if read_from_a else recipients_b[idx]
            ...
            for i in range(donor_count[idx]):
                ...
                if donor_idx >= 0:
                    # Read donor's state
                    donor_sign:ti.u64 = source_buffer[donor_idx] & ti.u64(0x80000000)
                    donor_iter:ti.u64 = source_buffer[donor_idx] & ti.u64(0x7FFFFFFF)
                    
                    donor_read_a = 1
                    if donor_iter == iteration:
                        donor_read_a = 1 - ((donor_sign >> 31) & 1)
                    else:
                        donor_read_a = (donor_sign >> 31) & 1
                    
                    donor_discharge = discharge_a[donor_idx] if donor_read_a else discharge_b[donor_idx]
                    donor_recipient = recipients_a[donor_idx] if donor_read_a else recipients_b[donor_idx]
                    ...
                    if donor_count_val == 0:
                        ...
                        updated = 1
                        # Mark donor as removed by setting recipient to -1
                        if donor_read_a:
                            recipients_b[donor_idx] = -1
                        else:
                            recipients_a[donor_idx] = -1
            ...
            if updated:
                ...
                new_sign = (1 - read_from_a) << 31
                source_buffer[idx] = new_sign | iteration
    ...
    def finalize_flow_routing(self, discharge_a: ti.template(), discharge_b: ti.template(),
        ...
        for idx in range(n_cells):
            sign_bit:ti.u64 = source_buffer[idx] & ti.u64(0x80000000)
            read_from_a = (sign_bit >> 31) & 1
            discharge[idx] = discharge_a[idx] if read_from_a else discharge_b[idx]
    ...
    def depression_carving_reverse(self, recipients: ti.template(), carving_tag: ti.template(),
        ...
        for idx in range(n_cells):
            r_idx = recipients[idx]
            if carving_tag[r_idx] == 1:
                # Check if current cell is not a local minimum
                is_local_min = 0
                for i in range(n_minima_val):
                    if local_minima[i] == idx:
                        is_local_min = 1
                        break
    ...
    def depression_routing(self, use_carving=True):
        ...
        max_iterations = int(math.log2(max(1, self.n_minima[None]))) + 1
    ...
    def run_fastflow(self, elevation_data, precipitation_data, outflow_data, depression = False):
        ...
        # Execute depression routing first
        if(depression):
...
if __name__ == "__main__":
    ...
    # Simple mountain with depression
    x = np.linspace(-2, 2, width)
    y = np.linspace(-2, 2, height)
    X, Y = np.meshgrid(x, y)
    elevation = 100 * np.exp(-(X**2 + Y**2)/2) # + 10 * np.random.rand(height, width)
    
    # Add artificial depression
    ...
    plt.imshow(np.log10(discharge), cmap = "Blues")
    # plt.imshow(fastflow.recipients.to_numpy().reshape(width,height), cmap = "magma")
    plt.show()
